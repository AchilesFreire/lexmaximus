...
TiposLicenca = (
 ...
 TConfiguracoes = class(TObject)
    private
      ...
      varExibirDicaBotoes               : Boolean;
      varUsarApenasUmClique             : Boolean;

      varCodigoProduto_LM               : Integer;
      varCodigoProduto_ES               : Integer;
      varAtualizacoesInstaladas         : String;
      varAtualizacoesArquivo            : String;
      varStringConexaoSrvAtu            : String;
    ...
    public
      ...
      property ExibirDicaBotoes               : Boolean  read varExibirDicaBotoes           write varExibirDicaBotoes ;

      property UsarApenasUmClique               : Boolean  read varUsarApenasUmClique           write varUsarApenasUmClique ;

      property CodigoProduto_LM               : integer  read varCodigoProduto_LM           write varCodigoProduto_LM ;
      property CodigoProduto_ES               : integer  read varCodigoProduto_ES           write varCodigoProduto_ES ;
      property AtualizacoesInstaladas                  : string  read varAtualizacoesInstaladas             write varAtualizacoesInstaladas;
      property StringConexaoSrvAtu      : string  read varStringConexaoSrvAtu write varStringConexaoSrvAtu;

      procedure CarregaConfiguracoes();
      procedure SalvaConfiguracoes();
      procedure SalvaConfiguracoesFormulario();
      procedure SalvaConfiguracoesLojaElfez();
...
begin
  ...
  varExibirDicaBotoes := True;
  varUsarApenasUmClique := False;
...
begin
  try
    ...
    temp                              := IniFile.ReadString  ( 'Geral'  , 'ExibirDicaBotoes'   , '' );
    varExibirDicaBotoes := ((temp = 'S') OR (temp = '')) ;

    temp                              := IniFile.ReadString  ( 'Geral'  , 'UsarApenasUmClique'   , '' );
    varUsarApenasUmClique := (temp = 'S')  ;
    ...
    varStringConexaoSrvAtu            := IniFile.ReadString( 'BancoDados'  , 'StringConexaoSrvAtu'   , '' );
    varCodigoProduto_ES               := IniFile.ReadInteger( 'LojaElfez'  , 'CodigoProduto_ES'   , 1 );
    varCodigoProduto_LM               := IniFile.ReadInteger( 'LojaElfez'  , 'CodigoProduto_LM'   , 2 );
    varAtualizacoesInstaladas        := IniFile.ReadString( 'LojaElfez'  , 'AtualizacoesInstaladas'   , '' );
    if AnsiStartsText(',', varAtualizacoesInstaladas) then
      varAtualizacoesInstaladas := Copy(varAtualizacoesInstaladas,2);

    if AnsiEndsText(',' , varAtualizacoesInstaladas) then
      varAtualizacoesInstaladas := Copy(varAtualizacoesInstaladas,1 , Length(varAtualizacoesInstaladas)-1);

    varAtualizacoesInstaladas := ReplaceStr(varAtualizacoesInstaladas, ',,' , ',');
    varAtualizacoesInstaladas := Trim(varAtualizacoesInstaladas);

    ArquivoInstalacoes := GetCurrentDir + '\Atualizacoes.txt';

    varAtualizacoesArquivo :='';

    if FileExists(ArquivoInstalacoes) then
      begin
        Conteudo := TStringList.Create();
        Conteudo.loadFromFile(ArquivoInstalacoes);

        if Conteudo.count <> 0 then
          begin
            varAtualizacoesArquivo := Conteudo.text;

            varAtualizacoesArquivo := ReplaceStr(varAtualizacoesArquivo, chr(13) + chr(10) , '');

            if AnsiStartsText(',', varAtualizacoesArquivo) then
              varAtualizacoesArquivo := Copy(varAtualizacoesArquivo,2);

            if AnsiEndsText(',' , varAtualizacoesArquivo) then
              varAtualizacoesArquivo := Copy(varAtualizacoesArquivo,1 , Length(varAtualizacoesArquivo)-1);

            varAtualizacoesArquivo := ReplaceStr(varAtualizacoesArquivo, ',,' , ',');
            varAtualizacoesArquivo := Trim(varAtualizacoesArquivo);

            if varAtualizacoesArquivo <> '' then
              begin
                if ( not ContainsText(varAtualizacoesInstaladas, varAtualizacoesArquivo) ) then
                  varAtualizacoesInstaladas := varAtualizacoesInstaladas + ',' + varAtualizacoesArquivo;
              end;
...
begin
  try
    ...
    IniFile.WriteString  ( 'LojaElfez'  , 'AtualizacoesInstaladas' , varAtualizacoesInstaladas);

    if varExibirDicaBotoes then
      temp := 'S'
    else
      temp := 'N';
    IniFile.WriteString  ( 'Geral' , 'ExibirDicaBotoes'           ,  temp );

     if varUsarApenasUmClique then
      temp := 'S'
    else
      temp := 'N';
    IniFile.WriteString  ( 'Geral' , 'UsarApenasUmClique'           ,  temp );
...
procedure TConfiguracoes.SalvaConfiguracoesLojaElfez();
var
  temp : string;
