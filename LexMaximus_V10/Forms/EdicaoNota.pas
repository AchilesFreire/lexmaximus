...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, StrUtils, ADODB, 
  Rul_Lei,
  Rul_NotaLei,
  CompressaoDados,
  cls_Conexao, Htmlview, ComCtrls, Buttons;
...
begin
  ...
  NotaLei.Conexao := self.Conexao.Conexao;
  NotaLei.Lei := Lei;
  NotaLei.posicao:= 0;
...
begin
  ...
  if MessageBox( self.Handle, 'Confirma a exclusão da nota selecionada?' , '' , MB_YESNO or MB_ICONQUESTION ) = IDYES then
    begin
      n := TRul_NotaLei.Create( Conexao.Conexao, Lei, Posicao);
...
begin
  ...
  nota := tRul_NotaLei.Create( Conexao.conexao , Lei, 0); // notaq de rodape = 0
...
begin
  try
    L := TRul_Lei.Create(self.Conexao.Conexao , lei);

    ArquivoComprimido := DiretorioHTML + '\' + L.Link;
    ArquivoComprimido := Copy(ArquivoComprimido, 1 , Length(ArquivoComprimido)-4);
    ArquivoComprimido := ArquivoComprimido + 'lmcf';
...
begin
  ...
  nota := tRul_NotaLei.Create( Conexao.conexao , Lei, PosicaoSelecionada);
...
begin
  ...
  NotaLei := TRul_NotaLei.Create(Conexao.Conexao, Lei , -1);
